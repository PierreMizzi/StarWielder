...
namespace PierreMizzi.Useful
{
	public static class UtilsClass
	{
		...
		public static List<T> PickRandom<T>(this List<T> list, int count, bool adjustCount = false)
		{
			List<T> randomList = new List<T>();

			if (list.Count == 0)
			{
				Debug.LogWarning("PickRandom : List is empty");
				return randomList;
			}
			else if (count == 0)
			{
				Debug.LogWarning("PickRandom : You sure you wanna pick 0 item in the list, buddy ?");
				return randomList;
			}
			else if (list.Count < count)
			{
				if (adjustCount == false)
				{
					Debug.LogWarning($"PickRandom : list has {list.Count} items, and you want {count} items. You sure, buddy ?");
					return randomList;
				}
				else
				{
